...
struct GPSTestScript {
    ...
    func runCompleteGPSTest() async {
        ...
        // 7. Manual Footsteps testen
        await testManualFootsteps()
    ...
    private func setupTestData() async {
        ...
        SampleDataCreator.printDataSummary(using: coreDataManager)
        
        await Task.sleep(nanoseconds: 1_000_000_000) // 1 Sekunde
    ...
    private func testPermissions() async {
        ...
        await Task.sleep(nanoseconds: 2_000_000_000) // 2 Sekunden
    ...
    private func testTrackingAccuracy() async {
        ...
        for accuracy in LocationManager.LocationAccuracy.allCases {
            ...
            await Task.sleep(nanoseconds: 500_000_000) // 0.5 Sekunden
        }
    ...
    private func testPauseDetection() async {
        ...
        await Task.sleep(nanoseconds: 1_000_000_000) // Verkürzt für Test
    ...
    private func testOfflineFunctionality() async {
        ...
        // Test-Footstep erstellen während "offline"
        locationManager.createManualFootstep(
            title: "Offline Test",
            content: "Footstep erstellt während Offline-Simulation",
            location: testLocations[0].clLocation
        )
        
        await Task.sleep(nanoseconds: 1_000_000_000)
        print("💾 Offline-Footstep erstellt")
        print("✅ Offline-Funktionalität getestet\n")
    }
    
    private func testManualFootsteps() async {
        print("📍 Test 7: Manuelle Footsteps")
        
        for (index, location) in testLocations.enumerated() {
            print("📌 Erstelle Footstep #\(index + 1): \(location.name)")
            
            locationManager.createManualFootstep(
                title: location.name,
                content: "Test-Footstep an \(location.name) erstellt",
        ...
        print("✅ Manuelle Footsteps erstellt\n")
    }
    ...
    func quickTest() {
        ...
        print("✅ Tracking gestartet für Trip: \(trip.title)")
        
        // Test-Footstep erstellen
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            self.locationManager.createManualFootstep(
                title: "Quick Test Location",
                content: "Schneller Test-Footstep",
                location: self.testLocations.randomElement()?.clLocation
            )
            print("✅ Test-Footstep erstellt")
        }
    }
    ...
    func stopTestAndShowResults() {
        ...
        let users = coreDataManager.fetchUsers()
        for user in users {
            let footsteps = coreDataManager.fetchFootsteps(for: user)
            print("👤 User \(user.displayName): \(footsteps.count) Footsteps")
        }
    }
}
